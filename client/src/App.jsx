...
export default function App() {
  ...
  useEffect(() => {
    ...
    socket.on('match_found', ({ roomId, playerRed, playerYellow, turn, board }) => {
      setRoomId(roomId);
    ...
    socket.on('player_disconnected', ({ username: disconnectedUser, graceSeconds }) => {
    ...
    socket.on('game_ended', ({ board: finalBoard, winner, reason, forfeiter: whoForfeited, newEloRed, newEloYellow }) => {
      setBoard(finalBoard);
    ...
    socket.on('game_restarted', ({ board: cleanBoard, turn: newTurn }) => {
      ...
      setRematchByMe(false);
      setRematchByOpponent(false);
      setMessages((prev) => [...prev, { system: true, text: '🎮 Rematch started!' }]);
    });

    socket.on('reconnect_failed', () => {
      // Clean up invalid local storage username
  ...
  const handleCellClick = (colIndex) => {
    if (gameWinner || gameEndedReason) return;
    if (turn !== username) return;
    if (opponentOffline) return;
  ...
  return (
    <div className="app-container">
      ...
      <header className="logo-container">
        ...
        <p className="logo-subtitle">Real-time online multiplayer Connect Four</p>
      </header>
      ...
      {screen === 'register' && (
        <div className="lobby-grid">
          ...
          <div className="glass-card">
            <h2 style={{ fontSize: '1.5rem', marginBottom: '1.25rem' }}>Welcome Player</h2>
            <form onSubmit={handleRegister} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              {errorMessage && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--color-red)', fontSize: '0.85rem' }}>
                  <AlertCircle size={16} />
          ...
          <div className="glass-card">
            ...
            <div className="leaderboard-list">
              {leaderboard.length === 0 ? (
                <p style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '20px 0' }}>No players registered yet.</p>
              ) : (
      ...
      {screen === 'queue' && (
        <div className="glass-card queue-box">
          ...
          <div>
            <h2 style={{ fontSize: '1.5rem', marginBottom: '8px' }}>Searching for Opponent</h2>
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>
              Finding a player close to your rank...
            </p>
          </div>
          <button onClick={leaveQueue} className="glass-btn glass-btn-secondary">
            Cancel Search
          </button>
        </div>
      )}
        ...
        <div className="game-grid">
          ...
          <div className="glass-card">
            <div className="game-status-header">
              <div>
                <span style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>Opponent</span>
              ...
              <div>
                {gameWinner || gameEndedReason ? (
                  ...
                  <span className={`status-badge badge-${myColor}`}>Your Turn</span>
                ) : (
                  <span className="status-badge" style={{ color: 'var(--text-secondary)' }}>Opponent's Turn</span>
                )}
              </div>
            </div>
            ...
            {opponentOffline && (
              <div style={{
                ...
                <ShieldAlert size={18} />
                <span>Opponent has disconnected. Waiting for them to reconnect...</span>
              </div>
            )}
              ...
              <div className="col-selector-row">
                {Array(7).fill(null).map((_, colIdx) => (
                  <button
                    ...
                    disabled={!isMyTurn || gameWinner !== null || opponentOffline}
            ...
            <div style={{ marginTop: '24px', display: 'flex', justifyContent: 'space-between' }}>
              ...
              <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                <span className="badge-active status-badge">
                  <span className={`piece-${myColor}`} style={{ width: '10px', height: '10px', borderRadius: '50%', display: 'inline-block', marginRight: '6px' }}></span>
                  You: {username}
                </span>
              </div>
            </div>
          ...
          <div className="glass-card chat-container">
            ...
            <div className="chat-messages">
              {messages.map((msg, idx) => (
                ...
                >
                  {!msg.system && (
                    <div className={`chat-sender ${msg.sender === username ? 'self' : 'opponent'}`}>
                      {msg.sender === username ? 'You' : msg.sender}
                    </div>
                  )}
                  <div style={{ wordBreak: 'break-word' }}>{msg.text}</div>
                  {!msg.system && <div className="chat-time">{msg.timestamp}</div>}
                </div>
              ))}
            ...
            <form onSubmit={handleSendMessage} className="chat-form">
              <input
                ...
                maxLength={80}
              />
              <button type="submit" className="glass-btn" style={{ padding: '12px' }}>
                <Send size={18} />
              </button>
            </form>
          </div>
      ...
      {(gameWinner !== null || gameEndedReason !== null) && (
        <div className="modal-overlay">
          <div className="glass-card modal-content">
            <h2 className={`modal-title ${gameWinner === username ? 'win' : ''}`}>
              {gameWinner === username ? '🎉 You Won!' : gameWinner === null ? '🤝 Draw Game' : '💀 Defeat'}
            </h2>
            
            <p style={{ color: 'var(--text-secondary)' }}>
              {gameEndedReason === 'forfeit' && (
                <span>{forfeiter === username ? 'You surrendered the match.' : `${opponentName} forfeited the match.`}</span>
              )}
              {gameEndedReason === 'timeout' && (
                <span>{forfeiter === username ? 'You ran out of time.' : `${opponentName} failed to reconnect.`}</span>
              )}
              {gameEndedReason === 'win' && (
                <span>{gameWinner === username ? 'Nice play! You got 4 in a row.' : 'Your opponent got 4 in a row.'}</span>
              )}
            ...
            {newRatings && (
              <div className="elo-change-box">
                ...
                <div style={{ color: 'var(--text-muted)', fontSize: '1.25rem' }}>vs</div>
                <div className="elo-node">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {!rematchByMe ? (
                ...
                <div className="status-badge badge-active" style={{ justifyContent: 'center', padding: '12px' }}>
                  Waiting for Opponent's Acceptance...
                </div>
              )}

              {rematchByOpponent && !rematchByMe && (
                <p style={{ fontSize: '0.85rem', color: 'var(--color-yellow)' }}>
                  ⚠️ Opponent requested a rematch! Click above to accept.
                </p>
              )}

              <button onClick={handleExitToLobby} className="glass-btn glass-btn-secondary">
                <LogOut size={18} />
                Exit to Lobby
              </button>
            </div>
